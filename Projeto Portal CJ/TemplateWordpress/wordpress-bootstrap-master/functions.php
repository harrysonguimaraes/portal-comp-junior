...
class Bootstrap_walker extends Walker_Nav_Menu{
  ...
  function start_el(&$output, $object, $depth = 0, $args = Array(), $current_object_id = 0){
    ...
    // Para inseriri a classe do menu
    if ( $args->has_children ) {
        $attributes .= ' class="dropdown-toggle borda-direita-Menu" data-toggle="dropdown"'; 
        $item_output .= '<a'. $attributes .'>';
    }else {
        if($object->ID == 40){
            $item_output .= '<a'. $attributes .'>';
        }else{
            $item_output .= '<a'. $attributes .' class="borda-direita-Menu">';
        }
    }
